...
namespace EMILtools.Core
{
    ...
    /// </summary>
    [Serializable]
    public sealed class PersistentFuncINOUT<T, TResult>
        where TResult : T
    {
        ...
        public PersistentFuncINOUT<T, TResult> Add(Func<T, T> cb)
        {
            _funcs.Add(cb);
            return this;
        }

        public PersistentFuncINOUT<T, TResult> Remove(Func<T, T> cb)
        {
            _funcs.Remove(cb);
            return this;
        }

        public PersistentFuncINOUT<T, TResult> Clear()
        {
            _funcs.Clear();
            return this;
        }

        public PersistentFuncINOUT<T, TResult> Add(params Func<T, T>[] cbs)
        {
            foreach (var cb in cbs) Add(cb);
            return this;
        }

        public PersistentFuncINOUT<T, TResult> Add(List<Func<T, T>> cbs)
        {
            foreach (var cb in cbs) Add(cb);
            return this;
        }
    ...
    {
        Func<T, TResult> _funcs = T => default;
            
        public PersistentFunc() { }
        public PersistentFunc(Func<T, TResult> cb) => _funcs = cb;
        public int Count => _funcs.GetInvocationList().Length;
        public TResult Invoke(T value) => _funcs(value);
    
        public PersistentFunc<T, TResult> Add(Func<T, TResult> cb) { _funcs += cb; return this; }
        public PersistentFunc<T, TResult> Remove(Func<T, TResult> cb)  { _funcs -= cb; return this; }
        public PersistentFunc<T, TResult> Clear()  { _funcs = T => default; return this; }
        public PersistentFunc<T, TResult> Add(params Func<T, TResult>[] cbs) {
            foreach (var cb in cbs) Add(cb); return this; }
        public PersistentFunc<T, TResult> Add(List<Func<T, TResult>> cbs) {
            foreach (var cb in cbs) Add(cb); return this; }
    }
}
